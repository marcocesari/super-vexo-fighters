// Queen Caza — queen of the Camelloo, the camel kingdom of the sand sea.
// Tall and regal in sand-gold silks, a high crescent headdress, turquoise
// jewels, and a long cloak that trails like a dune in the wind.
import { glow } from '../rig.js';

export default {
  id: 'caza', name: 'Queen Caza',
  tagline: 'Queen of the sand sea. Graceful, and the desert fights with her.',
  colours: { skin: '#c9905f', hair: '#2a1a10', suit: '#e8c377', armour: '#d9a441', armourLight: '#f2d58a',
             accent: '#2fb8c9', boots: '#7a4f2a', hands: '#c9905f', belt: '#2fb8c9', gold: '#f0c040' },
  ...
  proportions: { scale: 1.04, torsoW: 32, torsoD: 19, shoulderW: 19, armR: 6, legR: 7.5 },
  ...
  extras(p, j, P, C, ctx) {
    ...
    glow(p, C.accent, () => { p.translate(h.x + f * 12 * s, -h.y - 8 * s, h.z); p.sphere(3.2 * s, 8, 6); });
    // veil of hair down the back
    p.fill(C.hair); p.push(); p.translate(h.x - f * 10 * s, -h.y + 18 * s, h.z); p.box(8 * s, 40 * s, 20 * s); p.pop();
    // cloak, trailing behind
    const sway = Math.sin(ctx.t * 0.06) * 3 - ctx.fighter.vx * 2;
    p.fill('#f2d58a'); p.push(); p.translate(j.chest.x - f * 12 * s + sway * 0.3, -j.chest.y + 50 * s, -3); p.rotateZ(f * (0.16 + sway * 0.01)); p.box(5 * s, 110 * s, 40 * s); p.pop();
    // skirt and sash
